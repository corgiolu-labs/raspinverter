# -*- coding: utf-8 -*-
"""Lettura strutturata della config in-memory e merge/persistenza da POST `/api/config`."""
...
def build_config_get_payload() -> Dict[str, Any]:
    """Dizionario serializzabile come risposta GET `/api/config`."""
    return {
        "battery": {
            ...
            "soc": {
                ...
                "vmax_v": float(_get("battery.soc.vmax_v", 58.0))
                if _get("battery.soc.method", "voltage_based") == "voltage_based"
                else None,
                "vmin_v": float(_get("battery.soc.vmin_v", 44.0))
                if _get("battery.soc.method", "voltage_based") == "voltage_based"
                else None,
                "reset_voltage": float(_get("battery.soc.reset_voltage", 44.0))
                if _get("battery.soc.method", "voltage_based") == "energy_balance"
                else None,
            },
        },
...
def merge_config_from_post(data: Dict[str, Any]) -> Tuple[bool, Optional[str], bool]:
    """
    Applica il body JSON alla config in-memory.

    Returns:
        (success, error_message_or_none, changed)
    """
...
def finalize_config_persist(data: Dict[str, Any], changed: bool) -> bool:
    """
    Se `persist` e `changed`, rimuove webhook legacy dal relay, salva JSON e chiama `relay_setup`.
    Ritorna il flag `changed` aggiornato (True se sono stati rimossi webhook).
    """
    if not (data.get("persist") and changed):
        return changed
